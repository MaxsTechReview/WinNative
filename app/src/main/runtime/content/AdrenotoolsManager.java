...
public class AdrenotoolsManager {
  ...
  /**
   * Returns the original GitHub asset filename a driver was installed from, or an empty string if
   * the driver was installed before source-asset tracking existed (or from a local file picker,
   * where no asset name is available). Used to detect duplicate downloads in the Drivers screen.
   */
  ...
  private void reloadContainers(String adrenoToolsDriverId) {
    ...
    for (Container container : containerManager.getContainers()) {
      HashMap<String, String> config =
          GraphicsDriverConfigUtils.parseGraphicsDriverConfig(container.getGraphicsDriverConfig());
      String version = config.get("version");
      Log.d(
          "AdrenotoolsManager",
          "Checking if container driver version " + version + " matches " + driverName);
      if (version != null && driverName != null && version.contains(driverName)) {
        ...
        try {
          defaultVersion =
              GPUInformation.isDriverSupported(DefaultVersion.WRAPPER_ADRENO, mContext)
    ...
    for (Shortcut shortcut : containerManager.loadShortcuts()) {
      HashMap<String, String> config =
          GraphicsDriverConfigUtils.parseGraphicsDriverConfig(
              shortcut.getExtra(
                  "graphicsDriverConfig", shortcut.container.getGraphicsDriverConfig()));
      String version = config.get("version");
      Log.d(
          "AdrenotoolsManager",
          "Checking if shortcut driver version " + version + " matches " + driverName);
      if (version != null && driverName != null && version.contains(driverName)) {
        ...
        config.put("version", defaultVersion);
        shortcut.putExtra(
            "graphicsDriverConfig", GraphicsDriverConfigUtils.toGraphicsDriverConfig(config));
        shortcut.saveData();
      }
    }
  }
  ...
  public ArrayList<String> enumarateInstalledDrivers() {
    ...
    for (File f : files) {
      ...
      if (!fromResources && new File(f, "meta.json").exists()) driversList.add(f.getName());
    }
    return driversList;
  }
  ...
  /**
   * Installs a driver ZIP and, if {@code sourceAssetName} is non-null, stamps it into the driver's
   * meta.json as the {@code sourceAsset} field so the Drivers screen can later detect that a remote
   * GitHub asset with that filename is already installed.
   */
  public String installDriver(Uri driverUri, String sourceAssetName) {
    ...
    try {
      ...
      if (new File(tmpDir, "meta.json").exists()) {
        ...
        Log.d(
            "AdrenotoolsManager", "Failed to install driver, a valid driver has not been selected");
        tmpDir.delete();
      }
    } catch (IOException e) {

 ...
 */
class ContainerSettingsComposeDialog
    @JvmOverloads
    constructor(
        ...
        private fun loadInitialData() {
            ...
            state.selectedDInputMapperType.intValue =
                if ((inputType and WinHandler.FLAG_DINPUT_MAPPER_STANDARD.toInt()) ==
        ...
        private fun loadResourceArrays() {
            ...
            state.selectedMouseWarpOverride.intValue =
                when (pendingMouseWarpValue) {
                    "enable" -> 1
                    "force" -> 2
                    else -> 0
                }
                ...
                val defaultVersion =
                    try {
                        if (com.winlator.cmod.runtime.system.GPUInformation
                                .isDriverSupported(DefaultVersion.WRAPPER_ADRENO, context)
                        ) {
        ...
        private fun saveMouseWarpOverride(c: Container) {
            ...
            try {
                WineRegistryEditor(userRegFile).use { reg ->
                    val value =
                        when (state.selectedMouseWarpOverride.intValue) {
                            1 -> "enable"
                            2 -> "force"
                            else -> "disable"
                        }
        ...
        private fun loadGraphicsDriverConfigState() {
            ...
            selectByValue(
                state.gfxBcnEmulationTypeEntries.value,
                config.get("bcnEmulationType") ?: "compute",
                state.gfxSelectedBcnEmulationType,
            )
            selectByValue(
                state.gfxBcnEmulationCacheEntries.value,
                config.get("bcnEmulationCache") ?: "0",
                state.gfxSelectedBcnEmulationCache,
            )
        ...
        private fun buildGraphicsDriverConfigFromState(): String {
            ...
            val bcnEmulationType =
                state.gfxBcnEmulationTypeEntries.value.getOrElse(
                    state.gfxSelectedBcnEmulationType.intValue,
                ) { "compute" }
        ...
        private fun buildDxvkConfigFromState(): String {
            ...
            val ddrawWrapper =
                StringUtils.parseIdentifier(
                    state.dxvkDdrawWrapperEntries.value.getOrElse(
                        state.dxvkSelectedDdrawWrapper.intValue,
                    ) { Container.DEFAULT_DDRAWRAPPER },
                )

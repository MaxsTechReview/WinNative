...
public class WineInfo implements Parcelable {
    ...
    private static boolean hasX8664Hint(@Nullable String value) {
        if (value == null || value.isEmpty()) return false;
        String normalized = value.toLowerCase();
        return normalized.contains("x86_64") ||
                normalized.contains("x86-64") ||
                normalized.contains("x64-windows") ||
                normalized.contains("x86_64-windows");
    }
    ...
    private static String inferArch(@Nullable String identifier, @Nullable ContentProfile wineProfile, @Nullable File installDir) {
        if (installDir != null && installDir.isDirectory()) {
            File arm64UnixDir = new File(installDir, "lib/wine/aarch64-unix");
            if (arm64UnixDir.isDirectory()) return ARCH_ARM64EC;

            File arm64WindowsDir = new File(installDir, "lib/wine/aarch64-windows");
            File x8664Dir = new File(installDir, "lib/wine/x86_64-windows");
            if (x8664Dir.isDirectory()) return ARCH_X86_64;

            if (arm64WindowsDir.isDirectory()) return ARCH_ARM64EC;
        }

        if (hasArm64ECHint(identifier)) return ARCH_ARM64EC;
        if (hasX8664Hint(identifier)) return ARCH_X86_64;
        ...
        if (wineProfile != null) {
            ...
            if (hasX8664Hint(wineProfile.verName) ||
                    hasX8664Hint(wineProfile.wineLibPath) ||
                    hasX8664Hint(wineProfile.wineBinPath) ||
                    hasX8664Hint(wineProfile.winePrefixPack) ||
                    hasX8664Hint(ContentsManager.getEntryName(wineProfile))) {
                return ARCH_X86_64;
            }

            if ((installDir == null || !installDir.isDirectory()) && hasArm64ECHint(wineProfile.remoteUrl)) {
                return ARCH_ARM64EC;
            }
            if ((installDir == null || !installDir.isDirectory()) && hasX8664Hint(wineProfile.remoteUrl)) {
                return ARCH_X86_64;
            }
        }
        return ARCH_X86_64;

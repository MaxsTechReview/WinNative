...
public final class RefreshRateUtils {
    ...
    public static List<String> buildRefreshRateEntryLabels(Activity activity, @Nullable String leadingEntry) {
    ...
    public static int resolvePreferredDisplayModeId(Activity activity, int requestedHz) {
        ...
        for (Display.Mode mode : modes) {
            ...
            float delta = Math.abs(refreshRate - requestedHz);
            if (bestMode == null || delta < closestDelta || (delta == closestDelta && refreshRate > bestModeRate)) {
                bestMode = mode;
                bestModeRate = refreshRate;
                closestDelta = delta;
            }

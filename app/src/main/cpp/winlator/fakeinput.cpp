...
EXPORT ssize_t write(int fd, const void *buf, size_t count) {
    ...
    if (controller != controller_map.end() && count == sizeof(struct input_event)) {
        const auto *event = static_cast<const struct input_event *>(buf);
        uint16_t slot = static_cast<uint16_t>(get_event_number(controller->second));
        check_ff_event(event, slot);
        // FF commands are control traffic generated by Wine for rumble. Writing
        // them into the fake evdev stream makes Wine read them back as input.
        if (event->type == EV_FF)
    ...
    if (controller != controller_map.end()) {
        ...
        for (int i = 0; i < iovcnt; i++) {
            if (iov[i].iov_len == sizeof(struct input_event)) {
                const auto *event = static_cast<const struct input_event *>(iov[i].iov_base);
                check_ff_event(event, slot);
                if (event->type == EV_FF)
                    continue;
            }
        ...
        if (filtered.empty())
            return static_cast<ssize_t>(iovcnt * sizeof(struct input_event));
        return syscall(SYS_writev, fd, filtered.data(), filtered.size());
    }
